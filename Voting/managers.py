...
from .utils import instance_required, add_field_to_objects
...
class _VotableManager(models.Manager):
    def __init__(self, model, instance, field_name='votes'):
        self.model = model
    ...
    def vote(self, user_id, action):
        try:
            ...
            try:
                vote = Vote.objects.get(user_id=user_id,
    ...
    def delete(self, user_id):
        try:
            with transaction.atomic():
                ...
                self.instance = self.model.objects.select_for_update().get(pk=self.instance.pk)
                if vote.action in [UP, DOWN]:
                    vote.action = NO
                    vote.save()
                    if vote.action == UP:
                        up_field = Vote.ACTION_FIELD.get(UP)
    ...
    # @instance_required
    def exists(self, user_id, action=None):
        if action is None:
    ...
    def all(self, user_id, action=UP):
        ...
        object_ids = Vote.objects.filter(
            user_id=user_id,
            content_type=content_type,
    ...
    def count(self, action=UP):
        return Vote.votes_for(self.model,
                              self.instance, action).count()

    def user_ids(self, action=UP):
        return Vote.votes_for(
...
class VotableManager(GenericRelation):
    def __init__(self, **kwargs):
        ...
        super(VotableManager, self).__init__(Vote, **kwargs)
